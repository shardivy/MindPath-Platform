...
// import React, { useState, useEffect } from "react";import { useNavigate } from "react-router-dom";import {Form,Input,Button,Card,Typography,Row,Col,DatePicker,Select,Divider,ConfigProvider,message,} from "antd";import {UserOutlined,MailOutlined,LockOutlined,PhoneOutlined,} from "@ant-design/icons";import adminTheme from "../theme/adminTheme";import { useDispatch, useSelector } from "react-redux";import { fetchActivePrograms } from "../adminSlices/programSlice";import { fetchStreams } from "../adminSlices/streamSlice";import {sendOtp,resetOtpState,verifyOtpRegister,studentRegister,} from "../adminSlices/studentSlice";import { registerHH } from "../hhSlices/hhRegisterSlice";

// const { Title, Text } = Typography;const { Option } = Select;

// const StudentRegister = () => {const navigate = useNavigate();const dispatch = useDispatch();const [form] = Form.useForm();

// // Stateconst [parentMode, setParentMode] = useState("compact");const [parentExists, setParentExists] = useState(null);const [otpSent, setOtpSent] = useState(false);const [otpVerified, setOtpVerified] = useState(false);const [otpValue, setOtpValue] = useState("");const [parentMobileValue, setParentMobileValue] = useState("");const [parentEmailValue, setParentEmailValue] = useState("");const [specializationOptions, setSpecializationOptions] = useState([]);const [parentNameFromApi, setParentNameFromApi] = useState("");const genderOptions = ["Male", "Female", "Other"];

// const selectedProgram = Form.useWatch("program", form);const hideParentSection = selectedProgram === "Hand Holding Program";

// // Redux selectorsconst {sendOtpLoading,verifyOtpLoading,registerLoading} = useSelector((state) => state.student || {});const { activeList: programList, loading: programsLoading } = useSelector((state) => state.programs);const { streamList, loading: streamsLoading } = useSelector((state) => state.streams);

// const [classOptions] = useState(["8", "9", "10", "11", "12", "Engineering", "Medical","Law", "Design", "Commerce", "Arts", "BBA", "UG", "PG", "Others",]);

// const specializationMap = {"11": ["PCM (Physics, Chemistry, Mathematics)", "PCB (Physics, Chemistry, Biology)", "PCMB", "Commerce", "Arts / Humanities"],"12": ["PCM (Physics, Chemistry, Mathematics)", "PCB (Physics, Chemistry, Biology)", "PCMB", "Commerce", "Arts / Humanities"],"Engineering": ["Computer Science Engineering (CSE)", "Information Technology (IT)", "AI/ML", "Data Science", "Electronics & Telecommunication (ENTC)"],"Medical": ["MBBS", "BDS (Dental)", "BAMS (Ayurveda)", "BHMS (Homeopathy)", "BPT (Physiotherapy)"],"Law": ["Criminal Law", "Corporate Law", "Civil Law", "Constitutional Law"],"Design": ["Fashion Design", "Interior Design", "Graphic Design", "UI/UX Design"],"Commerce": ["B.Com General", "B.Com Accounting", "B.Com Finance", "CA", "CS"],"Arts": ["BA English", "BA Psychology", "BA Sociology", "BA History"],"BBA": ["Finance", "Marketing", "HR", "International Business"],};

// // Fetch programs and streamsuseEffect(() => {dispatch(fetchActivePrograms());dispatch(fetchStreams());}, [dispatch]);

// // Cleanup OTP stateuseEffect(() => {return () => dispatch(resetOtpState());}, [dispatch]);

// // Helperconst canSendOtp =parentMobileValue.length === 10 && parentEmailValue && !otpVerified && !otpSent;

// // Handlersconst handleParentCheck = () => {const mobile = parentMobileValue;const email = parentEmailValue;
...
// const handleSendOtp = () => {const mobile = parentMobileValue;const email = parentEmailValue;
...
// const handleVerifyOtp = () => {if (!parentEmailValue) {message.error("Enter a valid parent email");return;}if (!otpValue || otpValue.trim().length < 4) {message.error("Enter a valid OTP");return;}
...
// const onFinish = (values) => {const isHandHolding = values.program === "Hand Holding Program";
...
// // const onFinish = (values) => {//   if (!otpVerified) {//     message.error("Please verify parent mobile before creating account");//     return;//   }

// //   const payload = {//     student_name: values.studentName,//     dob: values.dob ? values.dob.format("YYYY-MM-DD") : null,//     student_email: values.email,//     student_mobile: values.mobile || "",//     study_class: values.class,//     specialization: values.specialization || "",//     stream: values.stream || "",//     parent_mobile: values.parentMobile,//     parent_email: values.parentEmail,//     parent_name: values.parentName,//     program: programList.find((p) => p.name === values.program)?.id,//     password: values.password,//     confirm_password: values.confirmPassword,//   };

// //   dispatch(studentRegister(payload))//     .unwrap()//     .then((res) => {//       message.success(res.message || "Account created successfully");//       navigate("/");//     })//     .catch((err) => message.error(err));// };

// return (<divstyle={{minHeight: "100vh",display: "flex",alignItems: "center",justifyContent: "center",padding: 20,background: "linear-gradient(180deg, #F8FAFC 0%, #EEF2FF 100%)",}}><Cardbordered={false}style={{width: "100%",maxWidth: 980,borderRadius: 24,overflow: "hidden",background: "linear-gradient(135deg, #1E40AF, #6b85db)",boxShadow: "0 30px 70px rgba(30, 64, 175, 0.35)",}}>{/* LEFT INFO PANEL */}
...
//           style={{
//             color: "#FFFFFF",
//             padding: window.innerWidth < 768 ? "0px 4px" : "70px 32px",
//             display: "flex",
//             flexDirection: "column",
//             justifyContent: "center",
//             // background:
//             //   "linear-gradient(180deg, #355E4B 0%, #476F5C 100%)",
//             position: "relative",
//             overflow: "hidden",
//             marginTop: window.innerWidth < 768 ? "-630px" : "-630px",
//           }}
//         >
//           {/* Floating Animation */}
//           {/* <div
...
//   src="/meditation.png" // place meditation image in public folder
//   alt="Meditation"
//   style={{
//     width: "220px",
...
//             }}
//           >
//             Create your account and connect with experts who
//             support your growth, healing, and transformation.
//           </Text>
...
//             {/* 🔥 PROGRAM FIRST */}
//             <Divider orientation="left">Select Coach/Therapy</Divider>

//             <Row gutter={16}>
//               <Col md={24}>
//                 <Form.Item
//                   label="Coach/Therapy Type"
//                   name="program"
//                   rules={[{ required: true }]}
//                 >
//                   <Select
...
//                       <Input
//                         size="large"
//                         placeholder="Student / Employee / Business / Homemaker"
//                       />
//                     </Form.Item>
//                   </Col>
//                 </Row>
...
//               <Input.TextArea
//                 rows={4}
//                 placeholder="Example: Stress Management, Career Guidance, Relationship Counselling, Anxiety Support, Personal Growth, Confidence Building, etc."
//               />
//             </Form.Item>
